    ...
    <script>
        ...
        $(document).on('click', '.open-chat', function() {
            ...
            success: function(messages) {
                ...
                for (const [date, msgs] of Object.entries(groupedMessages)) {
                    ...
                    const isDownloadable = /\.(pdf|doc|docx|xls|xlsx)$/i.test(message.media_path);
                    const isVideo = /\.(mp4|webm|mov)$/i.test(message.media_path);
                    const isAudio = /\.(oga)$/i.test(message.media_path);
                    ...
                    if (isVideo) {
                        ...
                        <div class="message ${message.sender_type}">
                            ...
                            <audio controls>
                                <source src="${message.media_path}" type="audio/ogg">
                                Your browser does not support the audio element.
                            </audio>
                            </div>
                            <small>${messageTime}</small>
                        </div>`;
                    } else if ((message.message && message.message.trim().toLowerCase() === "photo received") ||
                        (message.message && message.message.trim().toLowerCase() === "sticker received")) {
                        messageHtml = generateMediaHTML(message, isDownloadable, messageTime);
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                $('#messages-' + userId).scrollTop($('#messages-' + userId)[0].scrollHeight);
