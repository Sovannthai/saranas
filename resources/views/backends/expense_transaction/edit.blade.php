...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="editExpenseTransactionLabel">@lang('Edit Expense')</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
            <form action="{{ route('expense_transactions.update', ['expense_transaction' => $transaction->id]) }}"
                method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    ...
                    <div class="col-sm-12 mt-2">
                        ...
                        <textarea name="note" id="note" class="form-control"
                            placeholder="@lang('Enter optional note')">{{ $transaction->note }}</textarea>
                        @error('note')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
